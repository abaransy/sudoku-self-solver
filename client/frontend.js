...
const NUMBER_DIV_TO_NUMBER_ELEMENT_RATIO = 0.7;
...
const createAndAppendNumberElement = (number, i, j) => {
  ...
  numberElementContainer.style.gridColumnEnd =
    j < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (1 / 3)
      ? j + FIRST_ROW_OR_COLUMN_OFFSET_END
      : j < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (2 / 3)
      ? j + SECOND_MIDDLE_ROW_OR_COLUMN_OFFSET_END
      : j + THIRD_ROW_OR_COLUMN_OFFSET_END;
  numberElementContainer.style.gridColumnStart =
    j < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (1 / 3)
      ? j + FIRST_ROW_OR_COLUMN_OFFSET_START
      : j < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (2 / 3)
      ? j + SECOND_MIDDLE_ROW_OR_COLUMN_OFFSET_START
      : j + THIRD_ROW_OR_COLUMN_OFFSET_START;
  numberElementContainer.style.gridRowEnd =
    i < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (1 / 3)
      ? i + FIRST_ROW_OR_COLUMN_OFFSET_END
      : i < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (2 / 3)
      ? i + SECOND_MIDDLE_ROW_OR_COLUMN_OFFSET_END
      : i + THIRD_ROW_OR_COLUMN_OFFSET_END;
  numberElementContainer.style.gridRowStart =
    i < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (1 / 3)
      ? i + FIRST_ROW_OR_COLUMN_OFFSET_START
      : i < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN * (2 / 3)
      ? i + SECOND_MIDDLE_ROW_OR_COLUMN_OFFSET_START
      : i + THIRD_ROW_OR_COLUMN_OFFSET_START;
  ...
  numberDiv.style.fontSize = `${
    NUMBER_DIV_TO_NUMBER_ELEMENT_RATIO *
    document.querySelector(".number-element").clientWidth
  }px`;
...
const isValid = (row, col, num) => {
  ...
  const blkrow = Math.floor(row / 3) * 3,
    blkcol = Math.floor(col / 3) * 3; // Block no. is i/3, first element is i/3*3

  for (let i = 0; i < NUMBER_OF_ELEMENTS_PER_ROW_OR_COLUMN; i++) {
    if (
      currentPuzzle[i][col] === num ||
      currentPuzzle[row][i] === num ||
      currentPuzzle[blkrow + Math.floor(i / 3)][blkcol + (i % 3)] === num
    ) {
      return false;
    }
  }
...
const windowResizeCallback = () => {
  ...
  numberDivs.forEach((numberDiv) => {
    numberDiv.style.fontSize = `${
      NUMBER_DIV_TO_NUMBER_ELEMENT_RATIO * numberElementWidth
    }px`;
  });
};
...
const cleanup = async () => {
  ...
  const fadeOut = async () => {
    ...
    const volumeReduction = Math.min(
      VOLUME_REDUCTION_STEP,
      Math.abs(0 - audio.volume)
    );
